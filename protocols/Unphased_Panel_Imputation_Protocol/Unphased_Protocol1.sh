...
#
# This is unphased query imputation protocol. It does not include typed variant aggregation step.
# 
...
${time_exec} -o ${TIME_MEM_LOG} --append -f "Genotype R2\t"%e"\t"%M ./ProxyTyper.sh -genotype_R2 ${QUERY_targets}_variants.bed ${QUERY_LOCAL_IMPUTER_REF_PANEL_ID}_coords_deanonym ${QUERY_targets} ${chr_id}_UNPHASED_PROTOCOL1_accs.txt
./ProxyTyper.sh -summarize_accuracy ${chr_id}_UNPHASED_PROTOCOL1_accs.txt 0.001 0.005
./ProxyTyper.sh -summarize_accuracy ${chr_id}_UNPHASED_PROTOCOL1_accs.txt 0.005 0.01
./ProxyTyper.sh -summarize_accuracy ${chr_id}_UNPHASED_PROTOCOL1_accs.txt 0.01 0.05
./ProxyTyper.sh -summarize_accuracy ${chr_id}_UNPHASED_PROTOCOL1_accs.txt 0.05 1
