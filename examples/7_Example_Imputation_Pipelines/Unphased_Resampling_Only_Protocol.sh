...
#
# This script implements an unphased query imputation protocol. The basic idea is to skip typed allele hashing (since we don't have phasing information), add random bias on each typed variant on query site. 
# Reference performs resampling and use the same bias addition steps and also decompose untyped variants. Both sites use the coordinate/map anonymization.
# This is a simplification of the Unphased_Panel_Imputation_Protocol.sh script by commenting out the tag permutation mechanism.
# 
...
# Start from genocoded QUERY panel, which removes the phased encoding, i.e., haplocoding, from the genotype matrix.
...
# This is a simple trick to make implementation easier: We randomly phase and use a 0-vicinity hashing model, which is simply a 1-parameter model, see below.
# Note that if the vicinity is increased, the accuracies will drop substantially.
...
# ################################################################################################
# # Unphased protocol does not normally use augmenting.
...
#################################################################################################
# Unlike the phased imputation protocol, we do not have a local imputation step since we did not resample the query panel, i.e., what we got from the central server corresponds immediately to the query site's subjects.
#################################################################################################
...
${time_exec} -o ${TIME_MEM_LOG} --append -f "Genotype R2\t"%e"\t"%M ./ProxyTyper.sh -genotype_R2 ${QUERY_targets}_variants.bed ${QUERY_LOCAL_IMPUTER_REF_PANEL_ID}_coords_deanonym ${QUERY_targets} ${chr_id}_CUSTOM_UNPHASED_PROTOCOL1_accs.txt
./ProxyTyper.sh -summarize_accuracy ${chr_id}_CUSTOM_UNPHASED_PROTOCOL1_accs.txt 0.001 0.005
./ProxyTyper.sh -summarize_accuracy ${chr_id}_CUSTOM_UNPHASED_PROTOCOL1_accs.txt 0.005 0.01
./ProxyTyper.sh -summarize_accuracy ${chr_id}_CUSTOM_UNPHASED_PROTOCOL1_accs.txt 0.01 0.05
./ProxyTyper.sh -summarize_accuracy ${chr_id}_CUSTOM_UNPHASED_PROTOCOL1_accs.txt 0.05 1
