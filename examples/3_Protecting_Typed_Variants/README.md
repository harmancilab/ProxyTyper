...
```
This option takes the typed variant positions (which is immediately available to the reference panel) and runs the sliding window to generatet the permuted coordinates for the typed variants.
...
```
This mechanism plays an important role in unphased imptuation protocol that we present later. Up to 90% of typed variants in unphased protocol are shuffled within their vicinities.
