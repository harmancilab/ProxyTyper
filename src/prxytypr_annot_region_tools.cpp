...
vector<vector<t_annot_region*>*>** get_shuffled_merged_regions_per_profile(vector<vector<t_annot_region*>*>* regions_per_profile, int n_shufflings)
{
	...
	for(int i_shuff = 0; i_shuff < n_shufflings; i_shuff++)
	{
		...
		// Shuffle the indices.
		vector<int>* cur_shuff_indices = rng->fast_permute_indices(0, (int)pooled_regions->size());
	...
	for(int i_chr = 0; i_chr < (int)regs1_res->chr_ids->size(); i_chr++)
	{
		...
		if(i_chr2 < (int)regs2_res->chr_ids->size())
		{
			...
			for(int i_reg = 0; i_reg < (int)cur_reg1_regs->size(); i_reg++)
			{
				...
				int i_posn = locate_posn_region_per_region_starts(cur_reg1_regs->at(i_reg)->start, cur_reg2_regs, 0, (int)cur_reg2_regs->size()-1);
...
vector<t_annot_region*>* load_narrowPeak(char* narrowPeak_fp)
{
	...
	printf("Loaded %d regions from %s\n", (int)np_regions->size(), narrowPeak_fp);
	return(np_regions);
}
