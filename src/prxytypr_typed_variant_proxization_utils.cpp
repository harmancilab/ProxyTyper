...
void generic_map_regions(vector<t_annot_region*>* target_regs, char* source_coords_bed_fp, char* dest_coords_bed_fp)
{
	...
	fprintf(stderr, "Generic mapping coordiantes of %d regions using %d source regions to %d destination regions.\n", (int)target_regs->size(),
		(int)source_coord_regs->size(), (int)dest_coord_regs->size());
	...
	char* genetic_map_file, double cM_noise_SD, char* mapper_op_prefix, char* op_dir)
{
	...
	int cur_var_coord = 1;
	int delta_pos = 100*1000*1000 / ((int)tag_target_regs->size());
	...
	for (int i_reg = 0; i_reg < (int)anon_cM_tag_var_regs->size(); i_reg++)
	{
		fprintf(f_anon_genetic_map, "%d 0.000 %.4f\n",
	...
	sprintf(tag_target_orig_coords_fp, "%s/%s_original_coords.bed", op_dir, mapper_op_prefix);
	dump_BED(tag_target_orig_coords_fp, tag_target_orig_regs);

	char tag_target_mapping_coords_fp[1000];
	sprintf(tag_target_mapping_coords_fp, "%s/%s_mapping_coords.bed", op_dir, mapper_op_prefix);
...
void anonymize_genetic_distances(char* variants_BED_fp, char* genetic_map_file, double cM_noise_SD, char* op_fp)
{
	...
	for (int i_reg = 0; i_reg < (int)anon_dist_regs->size(); i_reg++)
	{
		...
		fprintf(f_op, "%d 0.000 %.4f\n",
				anon_dist_regs->at(i_reg)->end,
...
{
	//vector<t_annot_region*>* perm_var_orig_regs = load_BED(permuted_orig_vars_BED_fp);
	//vector<t_annot_region*>* perm_var_perm_regs = load_BED(permuted_perm_vars_BED_fp);
	vector<t_annot_region*>* perm_var_orig_regs = load_tag_permute_proxy_mapping_regs(permute_proxy_mappings_BED_fp);

	//for (int i_reg = 0; i_reg < (int)perm_var_orig_regs->size(); i_reg++)
	//{
	//	perm_var_orig_regs->at(i_reg)->data = perm_var_perm_regs->at(i_reg);
	//} // i_reg loop.
	...
	t_restr_annot_region_list* restr_perm_var_regs = restructure_annot_regions(perm_var_orig_regs);

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	vector<t_annot_region*>* permute_proxized_geno_sig_regs = new vector<t_annot_region*>();

	// Start processing the proxized genotypes.
	for (int i_chr = 0; i_chr < (int)restr_haplocoded_var_regs->chr_ids->size(); i_chr++)
	{
		fprintf(stderr, "Permutation-based Proxizing variants on %s\n", restr_haplocoded_var_regs->chr_ids->at(i_chr));
		vector<t_annot_region*>* cur_chr_perm_var_regs = restr_perm_var_regs->regions_per_chrom[i_chr];

		vector<t_annot_region*>* cur_chr_haplocoded_var_regs = restr_haplocoded_var_regs->regions_per_chrom[i_chr];

		fprintf(stderr, "Sorting the %d variants per position.\n", (int)cur_chr_haplocoded_var_regs->size());

		sort(cur_chr_haplocoded_var_regs->begin(), cur_chr_haplocoded_var_regs->end(), sort_regions);
		sort(cur_chr_perm_var_regs->begin(), cur_chr_perm_var_regs->end(), sort_regions);

		for (int i_var = 0; i_var < (int)cur_chr_haplocoded_var_regs->size(); i_var++)
		{
			if (cur_chr_haplocoded_var_regs->at(i_var)->start != cur_chr_perm_var_regs->at(i_var)->start)
			{
				fprintf(stderr, "Sanity check failed: coordinates do not match at %d. variant.\n", i_var);
				exit(0);
			}

			t_annot_region* cur_orig_var_perm_reg = (t_annot_region*)(cur_chr_perm_var_regs->at(i_var)->data);

			// Replace this variant.
			t_annot_region* located_perm_geno_sig_var_reg = NULL;
			for (int j_var = MAX(0, i_var - 1000); j_var < MIN(i_var + 1000, (int)cur_chr_haplocoded_var_regs->size()); j_var++)
			{
				if (cur_chr_haplocoded_var_regs->at(j_var)->start == cur_orig_var_perm_reg->start &&
					cur_chr_haplocoded_var_regs->at(j_var)->end == cur_orig_var_perm_reg->end)
				{
					located_perm_geno_sig_var_reg = cur_chr_haplocoded_var_regs->at(j_var);
					break;
				}
			} // j_var loop.

			if (located_perm_geno_sig_var_reg == NULL)
			{
				fprintf(stderr, "Could not find permuted region..\n");
				exit(0);
			}

			// Copy the original genotype signal region, add signal from located geno sig region.
			t_annot_region* new_sig_reg = duplicate_region(cur_chr_haplocoded_var_regs->at(i_var));
			new_sig_reg->data = located_perm_geno_sig_var_reg->data;
			void** reg_info = (void**)(located_perm_geno_sig_var_reg->data);
			char* geno_sig = (char* )(reg_info[0]);

			// Invert?
			if (cur_orig_var_perm_reg->score == 1)
			{
			...
			permute_proxized_geno_sig_regs->push_back(new_sig_reg);
		} // i_var loop.
	} // i_chr loop.

	// Save the genotype signals.
	binarize_variant_genotype_signal_regions(permute_proxized_geno_sig_regs, NULL, sample_ids, proxized_haplocoded_var_regs_op_fp);
...
void generate_permute_proxizing_parameters(char* haplocoded_geno_sig_regs_fp, char* sample_ids_list_fp, int n_vicinity, double geno_inversion_prob, char* op_BED_fp)
{
	...
	vector<char*>* sample_ids = buffer_file(sample_ids_list_fp);
	fprintf(stderr, "Loaded %d genotype regions, %d subjects.\n", (int)geno_sig_regs->size(), (int)sample_ids->size());

	assign_variant_AFs_to_var_regs(geno_sig_regs, sample_ids);
...
Relaxation delta N_e=%.4f\n", requested_normalized_N_e, delta_N_e_relaxation);
		...
		for (int i_reg = 0; i_reg < (int)geno_sig_regs->size(); i_reg++)
		{
			void** prev_reg_info = (void**)(geno_sig_regs->at(i_reg)->data);

			void** new_reg_info = new void* [10];
			new_reg_info[0] = prev_reg_info[0];
			new_reg_info[1] = prev_reg_info[1];
			new_reg_info[2] = prev_reg_info[2];
			new_reg_info[3] = prev_reg_info[3];
			...
			while ((n_first_order_weights + n_second_order_weights + n_third_order_weights) < min_n_params_per_var)
			{
				...
				// First, reset the first order weights.
				...
				for (int vic_i = 0; vic_i < (2 * n_vicinity + 1); vic_i++)
				{
					...
					// normalized_N_e tunes this option for selecting variants; small N_e always selects variants. High N_e only selects a small vicinity subset.
			...
			cur_reg_info[4] = per_var2var2var_interaction_weight;
		} // i_reg loop.
	} // i_chr loop.
	...
	for (int i_var = 0; i_var < (int)loaded_per_var_proxy_params->size(); i_var++)
	{
		...
		int*** per_var2var2var_weights = (int***)(cur_var_reg_info[4]);
...
// Save the parameters.
	...
	{
		void** cur_reg_info = (void**)(geno_sig_regs->at(i_reg)->data);
		int* per_var_weight = (int*)(cur_reg_info[2]);
		int** per_var2var_interaction_weight = (int**)(cur_reg_info[3]);
		int*** per_var2var2var_interaction_weight = (int***)(cur_reg_info[4]);
