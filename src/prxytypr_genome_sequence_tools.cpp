...
void batch_extract_region_sequences(char* chr_data_dir, vector<t_annot_region*>* regions)
{
	...
	for(int i_chr = 0; i_chr < (int)chr_ids->size(); i_chr++)
	{
		...
		if(check_file(chr_seq_fp))
		{
			...
			for(int i_reg = 0; i_reg < (int)cur_chr_regions->size(); i_reg++)
			{
				fprintf(stderr, "%d (%d)             \r", i_reg, (int)cur_chr_regions->size());
									...
									char* op_dir) // This is the number of matches to be traced per sequence.
{
	...
	if(negative_sequences != NULL)
	{
		fprintf(stderr, "Using %d positive sequences and %d negative sequences.\n", 
			(int)positive_sequences->size(),
			(int)negative_sequences->size());
	}
	else
	{
	...
	for(int i_seq = 0; i_seq < (int)positive_sequences->size(); i_seq++)
	{
	...
	fprintf(stderr, "Skipped %d positive sequences (%d)\n", n_pos_skipped_seqs, (int)positive_sequences->size());
	...
	{
		for(int i_seq = 0; 
			i_seq < (int)negative_sequences->size();
			i_seq++)
		{
		...
		fprintf(stderr, "Skipped %d sequences (%d)\n", n_skipped_neg_seqs, (int)negative_sequences->size());
	...
	for(int i_init = 0; i_init < n_inits; i_init++)
	{
		...
		for(int i_iter = 0; i_iter < n_iters_per_init; i_iter++)
		{
			...
			if((int)numerized_neg_sequences->size() > 0)
			{
...
void get_shuffled_motif_matching_statistics(char* sequence, double** kmer_emission_probs, int k, double& mean, double& std_dev, int n_randomizations)
{
	...
	if(n_randomizations != (int)all_scores->size())
	{
	...
	mean = 0.0;
	std_dev = 0.0;
	for(int i_score = 0; i_score < (int)all_scores->size(); i_score++)
	{
		mean += all_scores->at(i_score);
	} // i_score loop.
	mean /= (int)all_scores->size();

	for(int i_score = 0; i_score < (int)all_scores->size(); i_score++)
	{
		std_dev += (all_scores->at(i_score) - mean) * (all_scores->at(i_score) - mean);
	} // i_score loop.
	std_dev /= (int)all_scores->size();
								...
								int min_k, int max_k)
{
	...
	for(int k = min_k; k <= max_k; k++)
	{
		...
		if(numerized_neg_sequences == NULL || (int)numerized_neg_sequences->size() == 0)
		{
			// Get the nucleotide frequencies.
			double nuc_freqs[6];
			memset(nuc_freqs, 0, sizeof(double)*6);
			for(int i_seq = 0; i_seq < (int)numerized_pos_sequences->size(); i_seq++)
			{
			...
			for(int i_kmer = 0; i_kmer < (int)kmer_list->size(); i_kmer++)
			{
		...
		else if((int)numerized_neg_sequences->size() > 0)
		{
		...
		for(int i_kmer = 0; 
			i_kmer < MAX_KMER_PER_KMER_LENGTH && i_kmer < (int)cur_kmer_valid_raw_enriched_motifs->size();
			i_kmer++)
		{
		...
		double** distances = new double*[(int)valid_raw_enriched_motifs->size() + 2];
		for(int i_kmer = 0; i_kmer <= (int)valid_raw_enriched_motifs->size(); i_kmer++)
		{
			distances[i_kmer] = new double[(int)valid_raw_enriched_motifs->size()+2];
		} // i_kmer loop.
		...
		for(int i_kmer = 0; i_kmer < (int)valid_raw_enriched_motifs->size(); i_kmer++)
		{
			...
			for(int j_kmer = 0; j_kmer < (int)valid_raw_enriched_motifs->size(); j_kmer++)
			{
...
if(_DUMP_GENOME_SEQUENCE_MESSAGES_)
{
			...
			for(int i = 0; i < (int)valid_raw_enriched_motifs->size(); i++)
			{
				cur_avg_dist += distances[i_kmer+1][i+1];
			} // i loop/
			fprintf(f_avg_dists, "%s\t%lf\n", valid_raw_enriched_motifs->at(i_kmer)->kmer, cur_avg_dist/ (int)valid_raw_enriched_motifs->size());
		} // i_kmer loop.
		fclose(f_avg_dists);
	...
	for(int i_kmer = 0; i_kmer < (int)valid_raw_enriched_motifs->size(); i_kmer++)
	{
		...
		if((int)valid_raw_enriched_motifs->at(i_kmer)->k_mer_match_posns->size() != valid_raw_enriched_motifs->at(i_kmer)->count)
		{
			fprintf(stderr, "The number of matches does not match between the tree count and sequence search: %s: %d, %d\n", 
				...
				(int)valid_raw_enriched_motifs->at(i_kmer)->k_mer_match_posns->size(),
				valid_raw_enriched_motifs->at(i_kmer)->count);

			for(int i = 0; i < (int)valid_raw_enriched_motifs->at(i_kmer)->k_mer_match_posns->size(); i++)
			{
	...
	for(int i_kmer = 0; i_kmer < (int)valid_raw_enriched_motifs->size(); i_kmer++)
	{
		...
		for(int i_m = 0; i_m < (int)valid_raw_enriched_motifs->at(i_kmer)->k_mer_match_posns->size(); i_m++)
		{
		...
		for(int j_kmer = i_kmer+1; j_kmer < (int)valid_raw_enriched_motifs->size(); j_kmer++)
		{
			//int i_match;
			...
			else
			{
				...
				if(cur_kmers_match_score >= l_min_kmer*ALIGNMENT_SCORE_CUTOFF)
				{
					...
					for(int i_m = 0; i_m < (int)valid_raw_enriched_motifs->at(j_kmer)->k_mer_match_posns->size(); i_m++)
					{
...
t_kmer_count_tree_entry* build_kmer_frequency_tree(vector<int*>* positive_num_seqs, vector<int>* positive_num_seq_lengths, int k)
{
	...
	for(int i_seq = 0; i_seq < (int)positive_num_seqs->size(); i_seq++)
	{
...
void dump_mutated_embedded_sequences_w_multiple_kmers(vector<char*>* kmers, vector<double>* kmer_fractions, int l_seqs, int n_seqs, double mutation_prob, char* op_fp)
{
	...
	for(int k_i = 0; k_i < (int)kmers->size(); k_i++)
	{
		...
		for(int i_seq = 0; i_seq < nseqs_per_kmers->at(k_i); i_seq++)
		{
			...
			for(int i = 0; i < cur_rand_kmer_posn; i++)
			{
				...
				cur_seq[i_nuc] = nucs[(int)cur_nuc_i];
				i_nuc++;
			} // i loop.
	...
	vector<int>* permuted_indices = rng->permute_indices((int)sequences->size(), (int)sequences->size());

	// Dump the sequences.
	FILE* f_op = open_f(op_fp, "w");
	for(int i_seq = 0; i_seq < (int)sequences->size(); i_seq++)
	{
...
void dump_embedded_sequences_w_kmer(char* kmer, int l_seqs, int n_seqs, double mutation_prob, char* op_fp)
{
	...
	for(int i_seq = 0; i_seq < n_seqs; i_seq++)
	{
		...
		for(int i = 0; i < cur_rand_kmer_posn; i++)
		{
			...
			fprintf(f_op, "%c", nucs[(int)cur_nuc_i]);
		} // i loop.
