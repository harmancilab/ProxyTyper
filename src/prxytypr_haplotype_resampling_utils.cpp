...
const bool __DUMP_PROXYGIMP_MSGS__ = false;

//#define MAX(a,b) (((a)>(b))?(a):(b))
//#define MIN(a,b) (((a)<(b))?(a):(b))

//void extract_imputed_genotypes_per_resampling_patterns(char* resampling_hap_info_sigbed_fp, char* resampling_sample_list_fp, 
//	char* generating_sample_list_fp, 
//	char* imputed_targets_matbed_fp,
//	char* op_fp)
//{
//	vector<t_annot_region*>* resampling_pattern_regs = load_resampling_pattern_signal_var_regs(resampling_hap_info_sigbed_fp);
//	vector<t_annot_region*>* imputed_target_geno_regs = load_variant_signal_regions_wrapper(imputed_targets_matbed_fp, resampling_sample_list_fp);
//
//	vector<char*>* generating_sample_ids = buffer_file(generating_sample_list_fp);
//	vector<char*>* proxy_sample_ids = buffer_file(resampling_sample_list_fp);
//
//	for (int i_reg = 0; i_reg < imputed_target_geno_regs->size(); i_reg++)
//	{
//		void** cur_var_info = (void**)(imputed_target_geno_regs->at(i_reg)->data);
//		char* deconv_imputed_geno_sig = new char[vecsize(generating_sample_ids)];
//
//		cur_var_info[1] = deconv_imputed_geno_sig;
//	} // i_reg loop.
//
//	// These are the tag variants.
//	for (int i_reg = 0; i_reg < vecsize(resampling_pattern_regs); i_reg++)
//	{
//		// For each proxy haplotype, check which generating subject it comes from.
//		for (int i_proxy_subj = 0; i_proxy_subj < vecsize(proxy_sample_ids); i_proxy_subj++)
//		{
//			for (int i_proxy_hap = 0; i_proxy_hap < 2; i_proxy_hap++)
//			{
//				void** cur_var_info = (void**)(resampling_pattern_regs->at(i_reg)->data);
//				int** per_hap_resampled_hap_i = (int**)(cur_var_info[0]);
//
//				int cur_haplo_state = per_hap_resampled_hap_i[i_proxy_hap][i_proxy_subj];
...
vector<t_annot_region*>* load_resampling_pattern_signal_var_regs(char* resampling_pattern_sig_BED_fp)
{
	...
	int n_regs = -1;
	fread(&(n_regs), sizeof(int), 1, f_resamp_regs);

	int n_resampled_sample_size = -1;
	fread(&(n_resampled_sample_size), sizeof(int), 1, f_resamp_regs);
	...
	char* op_prefix)
{
	...
	double min_cM_delta_per_recomb = 0.001;
	...
	char* op_prefix)
{
	...
	for (int i_thread = 0; i_thread < n_threads; i_thread++)
	{
		...
		fprintf(stderr, "Started %d. thread..         \r", i_thread);
	} // i_thread loop.
